...
def grafo_busca_profundidade(no_inicio, no_destino):
    fila = [no_inicio]
    fila[0].dist = 0
    ...
    while len(fila) > 0:
        ...
        for k in range(num_vizinhos):
            if fila[0].vizinhos[k].visitado == NAO:
                ...
                fila_vizinhos.append(fila[0].vizinhos[k])

        fila[0].visitado = SIM
        fila.pop(0)
        fila = fila_vizinhos + fila
...
def main():
    ORIGEM = 0
    DESTINO = 8
    grafo_busca_profundidade(nos[ORIGEM], nos[DESTINO])
